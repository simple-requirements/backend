...
async function seedE2eAuthenticationWithManager(manager: EntityManager): Promise<void> {
    const passwordHash = await hashPassword(E2E_LOGIN_PASSWORD, { type: argon2id });
    ...
    for (const user of E2E_USERS) {
       ...
       ON CONFLICT (id) DO UPDATE SET
           ...
           updated_at = NOW()`,
            [user.id, user.username, user.username, user.email, user.email, user.displayName, passwordHash, user.role],
        );
