    ...
    async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.createTable(
            new Table({
                ...
                uniques: [{ name: 'UQ_global_user_roles_user_role', columnNames: ['user_id', 'role'] }],
                checks: [{ name: 'CHK_global_user_roles_role', expression: `"role" IN ('administrator')` }],
                foreignKeys: [
                    {
        ...
        await queryRunner.createTable(
            new Table({
                ...
                columns: [
                    ...
                    { name: 'administrator_user_id', type: 'uuid', isUnique: true },
                    { name: 'completed_at', type: 'timestamptz' },
                    { name: 'created_at', type: 'timestamptz', default: 'now()' },
                ],
                checks: [{ name: 'CHK_authentication_bootstrap_singleton', expression: `"id" = 1` }],
                foreignKeys: [
                    {
