...
export class CategoriesService {
  ...
  async create(
    ...
    if (existingCategory !== null) {
      throw new ConflictException(
        `Category key "${createCategoryDto.key}" already exists`,
      );
    }
  ...
  private validateCreateCategoryDto(
    createCategoryDto: CreateCategoryDto,
  ): void {
    ...
    if (createCategoryDto.name.trim().length > 120) {
      throw new BadRequestException(
        'Category name must be no longer than 120 characters',
      );
    }
    ...
    if (createCategoryDto.key.length > 40) {
      throw new BadRequestException(
        'Category key must be no longer than 40 characters',
      );
    }
