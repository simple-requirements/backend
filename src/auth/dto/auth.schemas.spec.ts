...
describe('registerUserSchema', () => {
    ...
    it.each([
        ...
        [{ ...VALID_REGISTRATION, displayName: '   ' }, 'Display name must not be empty.'],
        [{ ...VALID_REGISTRATION, password: 'too short' }, 'Password must contain at least 15 characters.'],
    ])('rejects malformed registration data.', (registration, expectedMessage) => {
...
describe('role and password recovery schemas', () => {
    it('accepts one account role and valid recovery inputs.', () => {
        expect(updateUserRoleSchema.parse({ role: AccountRole.RequirementsEngineer })).toEqual({
            role: AccountRole.RequirementsEngineer,
        });
        expect(requestPasswordResetSchema.parse({ email: ' USER@Example.org ' })).toEqual({
            email: 'user@example.org',
        });
        expect(
            confirmPasswordResetSchema.safeParse({ token: 'token', password: 'correct horse battery staple' }).success,
        ).toBe(true);
    });

    it('rejects invalid roles, invalid email, and weak reset passwords.', () => {
        expect(updateUserRoleSchema.safeParse({ role: 'reviewer' }).success).toBe(false);
        expect(requestPasswordResetSchema.safeParse({ email: 'invalid' }).success).toBe(false);
        expect(confirmPasswordResetSchema.safeParse({ token: 'token', password: 'short' }).success).toBe(false);
    });
});

describe('email verification schemas', () => {
    it('trims valid confirmation tokens and resend usernames.', () => {
        expect(confirmEmailVerificationSchema.parse({ token: '  secure-token  ' })).toEqual({ token: 'secure-token' });
        expect(resendEmailVerificationSchema.parse({ username: '  Florian.W  ' })).toEqual({ username: 'Florian.W' });
    });
    ...
    it('rejects empty confirmation tokens and malformed usernames.', () => {
        ...
        expect(resendEmailVerificationSchema.safeParse({ username: 'invalid user' }).success).toBe(false);
    });
});
...
describe('bootstrapAdministratorSchema', () => {
    it('normalizes identity attributes and permits a missing secret for generic rejection.', () => {
        expect(bootstrapAdministratorSchema.parse({ ...VALID_REGISTRATION, bootstrapSecret: undefined })).toEqual({
            ...VALID_REGISTRATION,
            email: 'florian@example.org',
    ...
    it('validates bootstrap registration account fields.', () => {
        expect(bootstrapAdministratorSchema.safeParse({ ...VALID_REGISTRATION, password: 'too short' }).success).toBe(
            false,
        );
    });
});
...
describe('session and administration schemas', () => {
    it('accepts trimmed login credentials and supported account states.', () => {
        expect(loginSchema.parse({ username: ' Admin ', password: 'secret' })).toEqual({
            username: 'Admin',
            password: 'secret',
        });
        expect(updateUserStatusSchema.parse({ status: UserStatus.Active })).toEqual({ status: UserStatus.Active });
        expect(updateUserStatusSchema.parse({ status: UserStatus.Deactivated })).toEqual({
    ...
    it('rejects empty credentials and pending as an administrative target state.', () => {
        ...
        expect(updateUserStatusSchema.safeParse({ status: UserStatus.Pending }).success).toBe(false);
    });
});
