...
import { AccountRole, isProjectAccountRole } from '@/auth/accounts/account-role.enum';
...
export class ProjectMembershipService {
    ...
    async list(projectId: string): Promise<ProjectMembershipResponseDto[]> {
        ...
        const memberships = await this.dataSource
            ...
            .find({ where: { projectId }, relations: { user: true }, order: { createdAt: 'ASC' } });
    ...
    async listForUser(userId: string): Promise<AuthenticatedProjectMembershipDto[]> {
        const memberships = await this.dataSource
            .getRepository(ProjectMembership)
    ...
    async projectIdsForUser(userId: string): Promise<string[]> {
        const memberships = await this.dataSource.getRepository(ProjectMembership).findBy({ userId });
        return memberships.map(({ projectId }) => projectId);
    }

    async set(projectId: string, userId: string): Promise<ProjectMembershipResponseDto> {
        await this.requireProject(projectId);
        ...
        if (user.status !== UserStatus.Active) {
            throw new BadRequestException('Only active users can receive project memberships.');
        }
        if (user.role === AccountRole.Administrator) {
            throw new BadRequestException('Administrator accounts cannot receive project memberships.');
        }
        ...
        return { userId, username: user.username, displayName: user.displayName, role: user.role };
    }

    async remove(projectId: string, userId: string): Promise<void> {
        await this.requireProject(projectId);
        await this.dataSource.getRepository(ProjectMembership).delete({ projectId, userId });
    }
