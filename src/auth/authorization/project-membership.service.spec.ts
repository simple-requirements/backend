...
function account(role: AccountRole | null, status = UserStatus.Active): User {
    return Object.assign(new User(), { id: 'user-id', username: 'member', displayName: 'Member', role, status });
}
...
describe('ProjectMembershipService single-role memberships', () => {
    ...
    const service = new ProjectMembershipService(dataSource as unknown as DataSource);
    ...
    it('creates membership without storing a separate project role.', async () => {
        users.findOneBy.mockResolvedValue(account(AccountRole.RequirementsEngineer));
        ...
        expect(memberships.create).toHaveBeenCalledWith({ projectId: 'project-id', userId: 'user-id' });
    });
    ...
    it('rejects Administrator memberships.', async () => {
        ...
        await expect(service.set('project-id', 'user-id')).rejects.toEqual(
            new BadRequestException('Administrator accounts cannot receive project memberships.'),
        );
    ...
    it('rejects memberships for accounts that are not active.', async () => {
        users.findOneBy.mockResolvedValue(account(AccountRole.Viewer, UserStatus.Pending));

        await expect(service.set('project-id', 'user-id')).rejects.toEqual(
            new BadRequestException('Only active users can receive project memberships.'),
        );
    });
});
