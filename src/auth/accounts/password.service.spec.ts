...
describe('PasswordService', () => {
    ...
    it('creates an Argon2id hash that verifies the original password.', async () => {
        ...
        await expect(passwordService.verify(passwordHash, password)).resolves.toBe(true);
        await expect(passwordService.verify(passwordHash, 'different password')).resolves.toBe(false);
    });
