...
export class SessionService {
    ...
    async login(login: LoginDto, requestSource: string): Promise<LoginResponseDto> {
        ...
        const result = await this.dataSource.transaction(async (manager) => {
            await manager.query('SELECT pg_advisory_xact_lock(hashtext($1))', [`${sourceHash}:${usernameHash}`]);
            ...
            const user = await manager.getRepository(User).findOneBy({ normalizedUsername });
    ...
    async authenticate(plaintextToken: string): Promise<AuthenticatedPrincipal> {
        ...
        const principal = await this.dataSource.transaction(async (manager) => {
            ...
            const expired = session.lastActivityAt.getTime() <= now.getTime() - SESSION_INACTIVITY_MS;

            if (session.revokedAt !== null || expired || session.user.status !== UserStatus.Active) {
            ...
            return { user: session.user, session, role: session.user.role } satisfies AuthenticatedPrincipal;
        });
    ...
    private async createSession(manager: EntityManager, user: User): Promise<LoginResponseDto> {
        const generatedToken = this.tokenService.generate();
        const sessions = manager.getRepository(AuthenticationSession);
        const now = new Date();
        await sessions.save(
            sessions.create({ userId: user.id, tokenHash: generatedToken.hash, lastActivityAt: now, revokedAt: null }),
        );
        if (user.role === null) {
            throw new UnauthorizedException(GENERIC_LOGIN_FAILURE);
        }
