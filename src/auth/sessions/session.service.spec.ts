...
describe('SessionService login', () => {
    function setup(user: User | null, passwordMatches: boolean) {
        ...
        const sessions = { create: vi.fn((value) => value), save: vi.fn().mockResolvedValue(undefined) };
        const manager = {
        ...
        const passwordService = { verify: vi.fn().mockResolvedValue(passwordMatches) };
        const tokenService = {
            generate: vi.fn().mockReturnValue({ plaintext: 'opaque', hash: 'a'.repeat(64) }),
            hash: vi.fn(),
        };
    ...
    it('creates a session using only the generated token hash for an eligible user.', async () => {
        ...
        await expect(service.login({ username: ' USER ', password: 'password' }, '127.0.0.1')).resolves.toMatchObject({
            accessToken: 'opaque',
            user: { id: 'user-id' },
        });
    ...
    ])('returns the same failure for unknown, unavailable, and invalid credentials.', async (user, passwordMatches) => {
        const { service, attempts } = setup(user, passwordMatches);
        await expect(service.login({ username: 'user', password: 'wrong' }, '127.0.0.1')).rejects.toEqual(
