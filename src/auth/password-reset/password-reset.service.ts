...
const RESPONSE = Object.freeze({ message: 'If the account is eligible, a password reset email will be sent.' });
...
export class PasswordResetService {
    ...
    async request(email: string, source: string): Promise<{ message: string }> {
        ...
        if (
            (await attempts.count({ where: { sourceHash, createdAt: MoreThan(new Date(Date.now() - WINDOW_MS)) } }))
            >= 5
        )
            return RESPONSE;
        ...
        const user = await this.dataSource
            .getRepository(User)
            .findOneBy({ normalizedEmail: email.normalize('NFKC').toLocaleLowerCase('en-US') });
        if (!user?.emailVerifiedAt || user.status === UserStatus.Deactivated) return RESPONSE;
        const generated = this.tokens.generate();
        const repository = this.dataSource.getRepository(PasswordResetToken);
        await repository.update({ userId: user.id, consumedAt: IsNull() }, { consumedAt: new Date() });
        ...
        await this.email.sendPasswordReset(user.email, user.displayName, generated.plaintext);
        return RESPONSE;
    }
    ...
    async confirm(dto: ConfirmPasswordResetDto): Promise<void> {
        await this.dataSource.transaction(async (manager) => {
            ...
            const user = await manager.getRepository(User).findOneBy({ id: token.userId });
