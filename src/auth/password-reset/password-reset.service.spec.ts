...
describe('PasswordResetService', () => {
    function setup(user: unknown) {
        ...
        const tokens = {
            generate: vi.fn().mockReturnValue({ plaintext: 'plain-token', hash: 'hashed-token' }),
            hash: vi.fn(),
        };
    ...
    it('returns the same non-enumerating response for an unknown email.', async () => {
        ...
        await expect(service.request('missing@example.org', 'source')).resolves.toEqual({
    ...
    it('stores only a token hash and emails the plaintext token to an eligible user.', async () => {
        ...
        expect(resetTokens.create.mock.calls[0]?.[0]).not.toHaveProperty('plaintext');
        expect(email.sendPasswordReset).toHaveBeenCalledWith(user.email, user.displayName, 'plain-token');
    });
});
