...
vi.mock('nodemailer', () => ({
    default: { createTransport: mailMocks.createTransport.mockReturnValue({ sendMail: mailMocks.sendMail }) },
}));
...
describe('EmailDeliveryService', () => {
    it('sends the plaintext token only in the generated frontend link.', async () => {
        ...
        await service.sendEmailVerification('user@example.org', 'Florian', 'plaintext-token');
        ...
        expect(mailMocks.sendMail).toHaveBeenCalledWith(
            expect.objectContaining({
                ...
                text: expect.stringContaining('https://requirements.example/app/verify-email?token=plaintext-token'),
            }),
        );
    });
    ...
    it('does not create a transporter or send mail when delivery is explicitly disabled.', async () => {
        ...
        const service = new EmailDeliveryService({ ...CONFIGURATION, disabled: true });
