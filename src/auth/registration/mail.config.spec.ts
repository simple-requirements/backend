...
describe('mail configuration', () => {
    ...
    it('rejects incomplete or malformed production configuration.', () => {
        ...
        expect(loadMailConfiguration).toThrow('Required environment variable "SMTP_HOST" is not defined');

        vi.stubEnv('SMTP_HOST', 'smtp.example.org');
        vi.stubEnv('SMTP_PORT', 'not-a-port');
        expect(loadMailConfiguration).toThrow('SMTP_PORT must be a valid TCP port, but received "not-a-port"');
    });
});
