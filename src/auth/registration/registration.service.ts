...
const REGISTRATION_RESPONSE = Object.freeze({ message: 'Registration received.' });
...
export class RegistrationService {
    ...
    findByUsername(username: string): Promise<User | null> {
        return this.usersRepository.findOneBy({ normalizedUsername: normalizeIdentityAttribute(username.trim()) });
    }

    findByEmail(email: string): Promise<User | null> {
        return this.usersRepository.findOneBy({ normalizedEmail: normalizeIdentityAttribute(email.trim()) });
    }
    ...
    async activateUser(userId: string): Promise<User> {
        ...
        return this.usersRepository.save(Object.assign(user, { status: UserStatus.Active }));
    }

    async deactivateUser(userId: string): Promise<User> {
        const user = await this.updateUser(userId, { status: UserStatus.Deactivated });
        await this.sessions.revokeAllForUser(userId);
        return user;
    }

    async changePasswordHash(userId: string, passwordHash: string): Promise<User> {
        const user = await this.updateUser(userId, { passwordHash });
        await this.sessions.revokeAllForUser(userId);
        return user;
    }

    private async updateUser(userId: string, patch: Partial<Pick<User, 'passwordHash' | 'status'>>): Promise<User> {
        const user = await this.findById(userId);
