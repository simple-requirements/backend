...
const INVALID_TOKEN_MESSAGE = 'The email verification link is invalid or has expired.';
...
export class EmailVerificationService {
    ...
    async issueForUser(user: User): Promise<void> {
        const delivery = await this.dataSource.transaction((manager) => this.prepareForUser(manager, user));
    ...
    async confirm(plaintextToken: string): Promise<void> {
        ...
        await this.dataSource.transaction(async (manager) => {
            ...
            const bootstrap = await manager
                ...
                .findOneBy({ id: AUTHENTICATION_BOOTSTRAP_ID, administratorUserId: user.id });
            ...
            await tokenRepository.update(
                { userId: user.id, id: Not(token.id), consumedAt: IsNull(), invalidatedAt: IsNull() },
                { invalidatedAt: now },
            );
        });
    }
    ...
    async resend(username: string): Promise<{ message: string }> {
        const delivery = await this.dataSource.transaction(async (manager) => {
            ...
            if (mostRecentToken !== null && mostRecentToken.createdAt.getTime() > now.getTime() - RESEND_INTERVAL_MS) {
                return null;
            }

            const recentTokenCount = await tokenRepository.count({
                where: { userId: user.id, createdAt: MoreThan(new Date(now.getTime() - RESEND_WINDOW_MS)) },
            });

            return recentTokenCount >= MAX_EMAILS_PER_WINDOW ? null : this.prepareForUser(manager, user, now);
        });
    ...
    async prepareForUser(manager: EntityManager, user: User, now = new Date()): Promise<PendingVerificationDelivery> {
        const tokenRepository = manager.getRepository(EmailVerificationToken);
        ...
        return { email: user.email, displayName: user.displayName, plaintextToken: generatedToken.plaintext };
    }
    ...
    async deliver(delivery: PendingVerificationDelivery): Promise<void> {
        try {
            ...
            this.logger.error('Email verification delivery failed.', error instanceof Error ? error.stack : undefined);
        }
    }
}
