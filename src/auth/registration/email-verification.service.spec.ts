...
import { EmailVerificationService, RESEND_RESPONSE } from '@/auth/registration/email-verification.service';
...
function token(overrides: Partial<EmailVerificationToken> = {}): EmailVerificationToken {
...
describe('EmailVerificationService', () => {
    const tokenRepository = { count: vi.fn(), create: vi.fn(), findOne: vi.fn(), save: vi.fn(), update: vi.fn() };
    const userRepository = { findOne: vi.fn(), save: vi.fn() };
    const bootstrapRepository = { findOneBy: vi.fn() };
    const manager = {
    ...
    const dataSource = {
        transaction: vi.fn((callback: (transactionManager: EntityManager) => unknown) =>
            callback(manager as unknown as EntityManager),
        ),
    };
    const tokenService = { generate: vi.fn(), hash: vi.fn() };
    const emailDeliveryService = { sendEmailVerification: vi.fn() };
    let service: EmailVerificationService;

    beforeEach(() => {
        vi.clearAllMocks();
        tokenRepository.create.mockImplementation((value: Partial<EmailVerificationToken>) => value);
        tokenRepository.save.mockImplementation((value: EmailVerificationToken) => Promise.resolve(value));
        tokenRepository.update.mockResolvedValue({ affected: 1 });
        tokenRepository.count.mockResolvedValue(0);
        userRepository.save.mockImplementation((value: User) => Promise.resolve(value));
        bootstrapRepository.findOneBy.mockResolvedValue(null);
        tokenService.generate.mockReturnValue({ plaintext: 'plaintext-token', hash: 'a'.repeat(64) });
    ...
    it('activates the initial Administrator after email confirmation.', async () => {
        ...
        bootstrapRepository.findOneBy.mockResolvedValue({ id: 1, administratorUserId: USER_ID });
    ...
    it('stores only a hash with a 30-minute expiry and delivers the plaintext token.', async () => {
        ...
        expect(tokenRepository.create).toHaveBeenCalledWith(
            expect.objectContaining({ tokenHash: 'a'.repeat(64), consumedAt: null, invalidatedAt: null }),
        );
        const createdToken = tokenRepository.create.mock.calls[0]?.[0] as EmailVerificationToken;
        expect(createdToken).not.toHaveProperty('plaintextToken');
        expect(createdToken.expiresAt.getTime()).toBeGreaterThanOrEqual(createdAt + 30 * 60 * 1000);
    ...
    it('keeps the token usable and conceals delivery failures.', async () => {
        ...
        emailDeliveryService.sendEmailVerification.mockRejectedValueOnce(new Error('SMTP unavailable'));
    ...
    ])('rejects an %s token with the generic error.', async (_case, storedToken) => {
        tokenRepository.findOne.mockResolvedValue(storedToken);

        await expect(service.confirm('plaintext-token')).rejects.toEqual(
            new BadRequestException('The email verification link is invalid or has expired.'),
        );
    });
    ...
    it('issues a resend for an eligible pending account.', async () => {
        ...
        tokenRepository.findOne.mockResolvedValue(token({ createdAt: new Date(Date.now() - 2 * 60 * 1000) }));
    ...
    ])('does not resend for an account that is %s.', async (_case, storedUser) => {
        userRepository.findOne.mockResolvedValue(storedUser);
    ...
    it('enforces both the one-minute interval and five-per-hour limit.', async () => {
        userRepository.findOne.mockResolvedValue(user());
        tokenRepository.findOne.mockResolvedValueOnce(token({ createdAt: new Date() }));

        await service.resend('Florian.W');
        expect(tokenService.generate).not.toHaveBeenCalled();

        tokenRepository.findOne.mockResolvedValueOnce(token({ createdAt: new Date(Date.now() - 2 * 60 * 1000) }));
