...
function frontendBaseUrl(): string {
    const value =
        ...
        ?? (mailDeliveryIsDisabled() ? 'http://localhost:5173' : requiredEnvironmentVariable('FRONTEND_BASE_URL'));
...
function smtpPort(): number {
    ...
    if (!Number.isInteger(port) || port < 1 || port > 65_535) {
        throw new Error(`SMTP_PORT must be a valid TCP port, but received "${value}"`);
    }
...
export function loadMailConfiguration(): MailConfiguration {
    ...
    return {
        ...
        from: disabled ? 'test@example.invalid' : requiredEnvironmentVariable('SMTP_FROM'),
