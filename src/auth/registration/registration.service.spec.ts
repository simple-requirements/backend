...
describe('RegistrationService', () => {
    const usersRepository = { create: vi.fn(), findOneBy: vi.fn(), save: vi.fn() };
    ...
    it('creates a pending user with normalized identity attributes and a password hash.', async () => {
        await expect(service.register(REGISTRATION)).resolves.toEqual({ message: 'Registration received.' });
        ...
        expect(usersRepository.create.mock.calls[0]?.[0]).not.toHaveProperty('password');
    ...
    it('returns the same generic response for a uniqueness conflict.', async () => {
        const driverError = Object.assign(new Error('duplicate user'), { code: '23505' });
        usersRepository.save.mockRejectedValueOnce(new QueryFailedError('INSERT INTO users', [], driverError));
    ...
    it('does not hide unexpected persistence errors.', async () => {
        usersRepository.save.mockRejectedValueOnce(new Error('database unavailable'));

        await expect(service.register(REGISTRATION)).rejects.toThrow('database unavailable');
    });
    ...
    it('looks up usernames and emails case-insensitively.', async () => {
        ...
        expect(usersRepository.findOneBy).toHaveBeenNthCalledWith(1, { normalizedUsername: 'florian.w' });
        expect(usersRepository.findOneBy).toHaveBeenNthCalledWith(2, { normalizedEmail: 'florian@example.org' });
    });

    it.each([
        ['activate', UserStatus.Active],
        ['deactivate', UserStatus.Deactivated],
    ] as const)('can %s a stored user through the internal service.', async (operation, expectedStatus) => {
        ...
        const result =
            operation === 'activate' ? await service.activateUser(user.id) : await service.deactivateUser(user.id);

        expect(result.status).toBe(expectedStatus);
        expect(usersRepository.save).toHaveBeenCalledWith(user);
        expect(sessionService.revokeAllForUser).toHaveBeenCalledTimes(operation === 'deactivate' ? 1 : 0);
    });
    ...
    it('changes only the stored password hash through the internal service.', async () => {
        ...
        const updatedUser = await service.changePasswordHash(user.id, 'new-encoded-value');
    ...
    it('rejects internal updates for an unknown user.', async () => {
        ...
        await expect(service.activateUser('3a7f9e0c-8d9c-4a5f-a3d2-1a44a28e0d11')).rejects.toThrow(
