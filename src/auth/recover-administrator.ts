    ...
    try {
        await dataSource.transaction(async (manager) => {
            ...
            const user = await manager
                .getRepository(User)
                .findOneBy({ normalizedUsername: username.normalize('NFKC').toLocaleLowerCase('en-US') });
            if (user === null) throw new Error(`User "${username}" was not found.`);
            if (user.emailVerifiedAt === null) {
                throw new Error('The recovery account must have a verified email address.');
            }
            if (user.role !== AccountRole.Administrator) {
                throw new Error('Administrator recovery can only reactivate an existing Administrator account.');
            }
