...
const GENERIC_RESEND_RESPONSE = { message: 'If the account is eligible, a verification email will be sent.' };
const BOOTSTRAP_RESPONSE = { message: 'Bootstrap registration received.' };
const TEST_BOOTSTRAP_SECRET =
    process.env.INITIAL_ADMIN_BOOTSTRAP_SECRET ?? 'test-only-bootstrap-secret-with-32-characters';
...
async function findPersistedUser(normalizedUsername: string): Promise<PersistedUserRow> {
...
async function findVerificationTokens(
    userId: string,
): Promise<{ consumedAt: Date | null; invalidatedAt: Date | null; tokenHash: string }[]> {
    const result: unknown = await inspectionDataSource.query(
    ...
    return result as { consumedAt: Date | null; invalidatedAt: Date | null; tokenHash: string }[];
}

async function insertVerificationToken(userId: string, plaintextToken: string): Promise<void> {
...
async function countPersistedUsers(normalizedUsername: string): Promise<number> {
    const result: unknown = await inspectionDataSource.query(
...
async function countRows(tableName: string): Promise<number> {
    ...
    const result: unknown = await inspectionDataSource.query(`SELECT COUNT(*)::integer AS count FROM ${tableName}`);
...
async function createActiveAdministrator(request: APIRequestContext): Promise<{ accessToken: string; userId: string }> {
    const registration = { ...createRegistration(), bootstrapSecret: TEST_BOOTSTRAP_SECRET };
    expect((await request.post('/auth/bootstrap/administrator', { data: registration })).status()).toBe(202);
    ...
    expect(
        (await request.post('/auth/email-verification/confirm', { data: { token: verificationToken } })).status(),
    ).toBe(204);
...
test.describe('Authentication API - POST /auth/register', () => {
    ...
    test('accepts a valid pending registration without exposing account data.', async ({ request }) => {
        const registration = createRegistration();
        const response = await request.post('/auth/register', { data: registration });
        ...
        const persistedUser = await findPersistedUser(registration.username.toLowerCase());
        ...
        expect(JSON.stringify(body)).not.toContain(verificationTokens[0]?.tokenHash ?? 'unavailable-hash');
    });
    ...
    test('returns the generic response for duplicate username and email values regardless of casing.', async ({
        ...
        const registration = createRegistration();
        const firstResponse = await request.post('/auth/register', { data: registration });
        ...
        await expect(countPersistedUsers(registration.username.toLowerCase())).resolves.toBe(1);
    });
    ...
    test('keeps exactly one account during concurrent duplicate registration.', async ({ request }) => {
        const registration = createRegistration();
        const responses = await Promise.all(
            Array.from({ length: 4 }, () => request.post('/auth/register', { data: registration })),
        );
...
test.describe('Authentication API - email verification', () => {
    ...
    test('confirms a token once while the account remains pending.', async ({ request }) => {
        const registration = createRegistration();
        ...
        const response = await request.post('/auth/email-verification/confirm', { data: { token: plaintextToken } });
        expect(response.status()).toBe(204);

        const verifiedUser = await findPersistedUser(registration.username.toLowerCase());
        expect(verifiedUser.status).toBe('pending');
        expect(verifiedUser.emailVerifiedAt).not.toBeNull();
        expect((await findVerificationTokens(user.id)).at(-1)?.consumedAt).not.toBeNull();

        const reuseResponse = await request.post('/auth/email-verification/confirm', {
    ...
    test('returns a generic response and rotates an eligible resend token.', async ({ request }) => {
        const registration = createRegistration();
        await request.post('/auth/register', { data: registration });
        const user = await findPersistedUser(registration.username.toLowerCase());
        await inspectionDataSource.query(
        ...
        const unknownResponse = await request.post('/auth/email-verification/resend', {
...
test.describe('Authentication API - initial Administrator bootstrap', () => {
    ...
    test('registers, verifies, and permanently activates the first Administrator.', async ({ request }) => {
        const registration = createRegistration();
        ...
        const response = await request.post('/auth/bootstrap/administrator', {
            data: { ...registration, bootstrapSecret: TEST_BOOTSTRAP_SECRET },
        });
        expect(response.status()).toBe(202);
        ...
        await insertVerificationToken(user.id, plaintextToken);
        const verification = await request.post('/auth/email-verification/confirm', {
            data: { token: plaintextToken },
        });
        expect(verification.status()).toBe(204);
        const activeUser = await findPersistedUser(registration.username.toLowerCase());
        expect(activeUser.status).toBe('active');
        expect(activeUser.emailVerifiedAt).not.toBeNull();

        const completedStatus = await request.get('/auth/bootstrap/status');
        expect(await completedStatus.json()).toEqual({ registrationAvailable: false });
        await inspectionDataSource.query(`UPDATE users SET status = 'deactivated' WHERE id = $1`, [user.id]);
        const statusAfterDeactivation = await request.get('/auth/bootstrap/status');
        expect(await statusAfterDeactivation.json()).toEqual({ registrationAvailable: false });
    });

    test('rejects invalid secrets without creating partial bootstrap data.', async ({ request }) => {
        const response = await request.post('/auth/bootstrap/administrator', {
            data: { ...createRegistration(), bootstrapSecret: 'incorrect' },
        });

        expect(response.status()).toBe(403);
        expect(await response.json()).toMatchObject({ message: 'Bootstrap registration is not available.' });
    ...
    test('allows exactly one bootstrap result during concurrent attempts.', async ({ request }) => {
...
test.describe('Authentication API - sessions and user administration', () => {
    ...
    test('logs in with an opaque token, exposes the principal, and logs out.', async ({ request }) => {
        ...
        expect(rows[0]?.tokenHash).toBe(createHash('sha256').update(administrator.accessToken).digest('hex'));
    ...
    test("lets an Administrator activate and deactivate a verified user and revokes that user's sessions.", async ({
        ...
        const headers = { Authorization: `Bearer ${administrator.accessToken}` };
        const registration = createRegistration();
        expect((await request.post('/auth/register', { data: registration })).status()).toBe(202);
        const user = await findPersistedUser(registration.username.toLowerCase());
        await inspectionDataSource.query(`UPDATE users SET email_verified_at = NOW() WHERE id = $1`, [user.id]);
        const prematureActivation = await request.patch(`/admin/users/${user.id}/status`, {
            headers,
            data: { status: 'active' },
        });
        ...
        expect(await roleAssignment.json()).toMatchObject({ id: user.id, role: 'viewer' });
        ...
        expect(
            (await request.patch(`/admin/users/${user.id}/role`, { headers, data: { role: 'developer' } })).status(),
        ).toBe(409);
        const login = await request.post('/auth/login', {
            data: { username: registration.username, password: registration.password },
        });
        expect(login.status()).toBe(200);
        const userToken = ((await login.json()) as { accessToken: string }).accessToken;
        expect(
            (
                await request.patch(`/admin/users/${user.id}/status`, { headers, data: { status: 'deactivated' } })
            ).status(),
        ).toBe(200);
        expect((await request.get('/auth/me', { headers: { Authorization: `Bearer ${userToken}` } })).status()).toBe(
            401,
        );
    });

    test('lets an Administrator manage project memberships.', async ({ request }) => {
        const administrator = await createActiveAdministrator(request);
        const headers = { Authorization: `Bearer ${administrator.accessToken}` };
        ...
        expect(
            (
                await request.put(`/admin/projects/${project.id}/memberships/${administrator.userId}`, { headers })
            ).status(),
        ).toBe(400);
        const registration = createRegistration();
        await request.post('/auth/register', { data: registration });
        ...
        expect(
            (await request.patch(`/admin/users/${user.id}/status`, { headers, data: { status: 'active' } })).status(),
        ).toBe(200);
        const response = await request.put(`/admin/projects/${project.id}/memberships/${user.id}`, { headers });
        expect(response.status()).toBe(200);
        expect(await response.json()).toMatchObject({ userId: user.id, role: 'developer' });
        const login = await request.post('/auth/login', {
            data: { username: registration.username, password: registration.password },
        });
        const memberToken = ((await login.json()) as { accessToken: string }).accessToken;
        const memberMe = await request.get('/auth/me', { headers: { Authorization: `Bearer ${memberToken}` } });
        ...
        expect(
            (
                await request.get(`/projects/${project.id}`, { headers: { Authorization: `Bearer ${memberToken}` } })
            ).status(),
        ).toBe(200);
        expect(
            (
        ...
        expect(
            (await request.delete(`/admin/projects/${project.id}/memberships/${user.id}`, { headers })).status(),
        ).toBe(204);
        expect(
            (
                await request.get(`/projects/${project.id}`, { headers: { Authorization: `Bearer ${memberToken}` } })
    ...
    test('resets a verified account password once and revokes its sessions.', async ({ request }) => {
        const administrator = await createActiveAdministrator(request);
        ...
        const response = await request.post('/auth/password-reset/confirm', {
            data: { token: plaintextToken, password: 'new correct horse battery staple' },
        });
        expect(response.status()).toBe(204);
        expect(
            (
                await request.get('/auth/me', { headers: { Authorization: `Bearer ${administrator.accessToken}` } })
            ).status(),
        ).toBe(401);
        expect(
            (
                await request.post('/auth/password-reset/confirm', {
                    data: { token: plaintextToken, password: 'another correct horse battery staple' },
                })
            ).status(),
        ).toBe(400);
    });
