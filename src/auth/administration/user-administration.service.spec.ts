...
describe('UserAdministrationService role model', () => {
    const repository = { find: vi.fn(), findOneBy: vi.fn(), save: vi.fn((value: User) => Promise.resolve(value)) };
    const membershipRepository = { countBy: vi.fn() };
    const dataSource = {
        getRepository: vi.fn((entity: unknown) => (entity === ProjectMembership ? membershipRepository : repository)),
    };
    const sessions = { revokeAllForUser: vi.fn(), listForUser: vi.fn(), revokeSession: vi.fn() };
    ...
    it('assigns exactly one role while the account is pending.', async () => {
        ...
        await expect(
            service.assignRole('3a7f9e0c-8d9c-4a5f-a3d2-1a44a28e0d11', AccountRole.RequirementsEngineer),
    ...
    it('does not allow an active account to be converted into another role.', async () => {
        repository.findOneBy.mockResolvedValue(user({ status: UserStatus.Active, role: AccountRole.Viewer }));

        await expect(
            service.assignRole('3a7f9e0c-8d9c-4a5f-a3d2-1a44a28e0d11', AccountRole.Developer),
    ...
    it('requires a role before activation.', async () => {
        ...
        ).rejects.toEqual(new BadRequestException('An account role must be assigned before activation.'));
    });

    it('activates a verified pending account after a role was assigned.', async () => {
        repository.findOneBy.mockResolvedValue(user({ role: AccountRole.Developer }));
        ...
        ).resolves.toMatchObject({ status: UserStatus.Active, role: AccountRole.Developer });
    });
    ...
    it('allows an unassigned pending account to be deactivated.', async () => {
        ...
        ).resolves.toMatchObject({ status: UserStatus.Deactivated, role: null });
        expect(sessions.revokeAllForUser).toHaveBeenCalledWith('3a7f9e0c-8d9c-4a5f-a3d2-1a44a28e0d11');
    });

    it('blocks an Administrator from deactivating their own account.', async () => {
        const selfId = '3a7f9e0c-8d9c-4a5f-a3d2-1a44a28e0d11';
        repository.findOneBy.mockResolvedValue(user({ status: UserStatus.Active, role: AccountRole.Administrator }));

        await expect(service.updateStatus(selfId, UserStatus.Deactivated, selfId)).rejects.toEqual(
            new BadRequestException('Administrators cannot deactivate their own account.'),
        );
        expect(repository.save).not.toHaveBeenCalled();
    ...
    it('allows a deactivated account to change its project-scoped role.', async () => {
        repository.findOneBy.mockResolvedValue(
            user({ status: UserStatus.Deactivated, role: AccountRole.RequirementsEngineer }),
        );

        await expect(
            service.assignRole('3a7f9e0c-8d9c-4a5f-a3d2-1a44a28e0d11', AccountRole.Viewer),
        ).resolves.toMatchObject({ role: AccountRole.Viewer });
        expect(repository.save).toHaveBeenCalledWith(expect.objectContaining({ role: AccountRole.Viewer }));
    });

    it('allows a deactivated account without memberships to become Administrator.', async () => {
        repository.findOneBy.mockResolvedValue(user({ status: UserStatus.Deactivated, role: AccountRole.Viewer }));
        membershipRepository.countBy.mockResolvedValue(0);

        await expect(
            service.assignRole('3a7f9e0c-8d9c-4a5f-a3d2-1a44a28e0d11', AccountRole.Administrator),
        ).resolves.toMatchObject({ role: AccountRole.Administrator });
        expect(membershipRepository.countBy).toHaveBeenCalledWith({ userId: '3a7f9e0c-8d9c-4a5f-a3d2-1a44a28e0d11' });
    });
