...
import { updateUserRoleSchema, updateUserStatusSchema } from '@/auth/dto/auth.schemas';
...
export class UserAdministrationController {
    ...
    find(@Param('userId', ParseUUIDPipe) userId: string): Promise<UserAdministrationResponseDto> {
    ...
    @ApiOkResponse({ type: UserAdministrationResponseDto })
    @ApiConflictResponse({
        description: 'The account is active or Administrator assignment conflicts with existing project memberships.',
    })
    updateRole(
    ...
    updateStatus(
        ...
        return this.users.updateStatus(userId, update.status, request.authentication.user.id);
    }

    @Get(':userId/sessions')
    @ApiOkResponse({ type: SessionResponseDto, isArray: true })
    listSessions(@Param('userId', ParseUUIDPipe) userId: string): Promise<SessionResponseDto[]> {
    ...
    @ApiNoContentResponse()
    revokeAll(@Param('userId', ParseUUIDPipe) userId: string, @Req() request: AuthenticatedRequest): Promise<void> {
    ...
    revokeOne(
        ...
        return this.users.revokeSession(userId, sessionId, request.authentication.user.id);
    }
}
