...
import { BOOTSTRAP_RESPONSE, BootstrapService } from '@/auth/bootstrap/bootstrap.service';
...
const CONFIGURATION = { secret: 'test-only-bootstrap-secret-with-32-characters' } satisfies BootstrapConfiguration;
...
describe('BootstrapService', () => {
    const attemptRepository = { count: vi.fn(), create: vi.fn((value: object) => value), save: vi.fn() };
    const bootstrapRepository = { create: vi.fn((value: object) => value), existsBy: vi.fn(), save: vi.fn() };
    const userRepository = { create: vi.fn((value: object) => value), save: vi.fn() };
    const manager = {
    ...
    const emailVerificationService = { deliver: vi.fn(), prepareForUser: vi.fn() };
    ...
    beforeEach(() => {
        ...
        userRepository.save.mockImplementation((value: object) => Promise.resolve({ ...value, id: USER_ID }));
    ...
    it('reports bootstrap availability without exposing configuration.', async () => {
        await expect(service.status()).resolves.toEqual({ registrationAvailable: true });
        bootstrapRepository.existsBy.mockResolvedValueOnce(true);
        await expect(service.status()).resolves.toEqual({ registrationAvailable: false });
    });
    ...
    it('requires the deployment secret at startup only before completion.', async () => {
        ...
        await expect(serviceWithoutSecret.onApplicationBootstrap()).resolves.toBeUndefined();
    });

    it('atomically creates a pending Administrator and verification token.', async () => {
        await expect(service.registerAdministrator(REGISTRATION, '127.0.0.1')).resolves.toEqual(BOOTSTRAP_RESPONSE);
        ...
        expect(bootstrapRepository.create).toHaveBeenCalledWith(
            expect.objectContaining({ id: 1, administratorUserId: USER_ID, completedAt: expect.any(Date) }),
        );
        ...
        async (bootstrapSecret) => {
            await expect(
                service.registerAdministrator(
                    Object.assign(new BootstrapAdministratorDto(), REGISTRATION, { bootstrapSecret }),
                    '127.0.0.1',
                ),
            ).rejects.toEqual(new ForbiddenException('Bootstrap registration is not available.'));
    ...
    it('permanently rejects registration after bootstrap completion.', async () => {
        ...
        await expect(service.registerAdministrator(REGISTRATION, '127.0.0.1')).rejects.toEqual(
    ...
    it('rate-limits repeated attempts by a hashed request source.', async () => {
        ...
        await expect(service.registerAdministrator(REGISTRATION, '127.0.0.1')).rejects.toMatchObject({
            status: HttpStatus.TOO_MANY_REQUESTS,
        });
        expect(attemptRepository.create).toHaveBeenCalledWith({ sourceHash: expect.stringMatching(/^[a-f\d]{64}$/) });
        expect(passwordService.hash).not.toHaveBeenCalled();
    });

    it('does not send email when the atomic transaction fails.', async () => {
        bootstrapRepository.save.mockRejectedValueOnce(new Error('database unavailable'));

        await expect(service.registerAdministrator(REGISTRATION, '127.0.0.1')).rejects.toThrow('database unavailable');
        expect(emailVerificationService.deliver).not.toHaveBeenCalled();
    });
});
