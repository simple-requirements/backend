...
import { AUTHENTICATION_BOOTSTRAP_ID, AuthenticationBootstrap } from '@/auth/bootstrap/authentication-bootstrap.entity';
...
export const BOOTSTRAP_RESPONSE = Object.freeze({ message: 'Bootstrap registration received.' });
...
function isUniqueConstraintViolation(error: unknown): boolean {
    ...
    return (
        typeof driverError === 'object' && driverError !== null && 'code' in driverError && driverError.code === '23505'
    );
}
...
export class BootstrapService implements OnApplicationBootstrap {
    ...
    async onApplicationBootstrap(): Promise<void> {
        ...
        if (registrationAvailable && this.configuration.secret === null) {
            throw new Error('INITIAL_ADMIN_BOOTSTRAP_SECRET is required until bootstrap has completed');
        }
    }
    ...
    async registerAdministrator(
        ...
        try {
            delivery = await this.dataSource.transaction(async (manager) => {
                await manager.query("SELECT pg_advisory_xact_lock(hashtext('authentication-bootstrap'))");
                const bootstrapRepository = manager.getRepository(AuthenticationBootstrap);

                if (await bootstrapRepository.existsBy({ id: AUTHENTICATION_BOOTSTRAP_ID })) {
                ...
                const pendingDelivery = await this.emailVerificationService.prepareForUser(manager, user);
                const now = new Date();
    ...
    private assertSecret(candidate: string | undefined): void {
        ...
        if (this.configuration.secret === null || !timingSafeEqual(expectedDigest, candidateDigest)) {
    ...
    private async enforceAttemptLimit(requestSource: string): Promise<void> {
        ...
        const attemptCount = await this.dataSource.transaction(async (manager) => {
            await manager.query('SELECT pg_advisory_xact_lock(hashtext($1))', [sourceHash]);
            ...
            return repository.count({
                where: { sourceHash, createdAt: MoreThan(new Date(Date.now() - ATTEMPT_WINDOW_MS)) },
            });
        });

        if (attemptCount > MAX_ATTEMPTS_PER_WINDOW) {
            throw new HttpException(GENERIC_UNAVAILABLE_MESSAGE, HttpStatus.TOO_MANY_REQUESTS);
        }
    }
}
