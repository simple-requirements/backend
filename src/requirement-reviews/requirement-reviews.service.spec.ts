...
function createReviewCommentEntity(
  overrides: Partial<RequirementReviewComment> = {},
...
describe("RequirementReviewsService", () => {
  ...
  it("derives the in-review and decision-pending states from main comment counts.", async () => {
    ...
    reviewCommentsRepository.count
      .mockResolvedValueOnce(2)
      .mockResolvedValueOnce(1);

    await expect(
      service.getReviewSummary(PROJECT_ID, REQUIREMENT_ID),
    ).resolves.toEqual({
      commentCount: 2,
      openCommentCount: 1,
  ...
  it("approves a draft requirement when all review comments are closed.", async () => {
    ...
    const result = await service.approveRequirement(
      PROJECT_ID,
      REQUIREMENT_ID,
      { reviewer: "Jane Reviewer" },
    );
  ...
  it("rejects a draft requirement and auto-closes open comments.", async () => {
    ...
    expect(comment.closeReason).toBe(
