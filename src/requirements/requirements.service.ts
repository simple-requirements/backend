...
export class RequirementsService {
     ...
     * @throws BadRequestException If any filter is malformed or type and kind disagree.
     */
    private toListFilters(query: RequirementListQueryDto | boolean): RequirementListFilters {
        ...
        const type = query.type ?? query.kind;
        ...
        if (query.type !== undefined && query.kind !== undefined && query.type !== query.kind) {
            throw new BadRequestException('Requirement type and kind filters must match when both are provided');
        }
