...
const DEVELOPER_HEADERS = {
  Authorization: `Bearer ${E2E_DEVELOPER_ACCESS_TOKEN}`,
} as const;

const VIEWER_HEADERS = {
  Authorization: `Bearer ${E2E_VIEWER_ACCESS_TOKEN}`,
} as const;
...
test.describe("Domain conformance - authorization boundaries", () => {
  test("Administrator cannot access project content or receive project membership.", async ({
    request,
    api,
  }) => {
    const project = await api.createProject(
      `Conformance administrator isolation ${randomUUID()}`,
    );
    ...
    const membershipResponse = await request.put(
      `/admin/projects/${project.id}/memberships/${E2E_ADMIN_USER_ID}`,
  ...
  test("a project-scoped account loses access immediately when its membership is removed.", async ({
    ...
    const project = await api.createProject(
      `Conformance membership boundary ${randomUUID()}`,
    );
  ...
  test("Developer cannot mutate categories, requirement content, reviews, or lifecycle state.", async ({
    ...
    const categoryResponse = await request.patch(
    ...
    const requirementResponse = await request.patch(
      `/projects/${project.id}/requirements/${requirement.id}`,
      {
        data: {
          description: "Developer mutation",
          changeReason: "This request must be denied.",
        },
        headers: DEVELOPER_HEADERS,
      },
    );
    ...
    const reviewResponse = await request.post(
      ...
      {
        data: { text: "Developer review attempt" },
        headers: DEVELOPER_HEADERS,
      },
    );
  ...
  test("Viewer is read-only for assigned project content.", async ({
    request,
    draftRequirement,
  }) => {
...
test.describe("Domain conformance - lifecycle and revision boundaries", () => {
  test("generic requirement PATCH cannot approve or reject a draft.", async ({
    request,
    draftRequirement,
  }) => {
    const { project, requirement } = draftRequirement;
    ...
    const rejectResponse = await request.patch(
      `/projects/${project.id}/requirements/${requirement.id}`,
      {
        data: {
          ...
          rejectionReason: "Rejected through an invalid entry point.",
        },
        headers: E2E_REQUIREMENTS_ENGINEER_HEADERS,
      },
    );
  ...
  test("approved requirements remain approved after a substantive content edit.", async ({
    ...
    const updateResponse = await request.patch(
      ...
      {
        data: {
          description: "Updated approved wording.",
  ...
  test("Administrator API remains separate from project-scoped category creation.", async ({
    request,
    api,
  }) => {
    const project = await api.createProject(
      `Conformance administrator content boundary ${randomUUID()}`,
    );

    const response = await request.post(`/projects/${project.id}/categories`, {
      data: {
        name: "Administrator forbidden category",
        key: "AFC",
