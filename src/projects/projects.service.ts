...
export class ProjectsService {
    ...
    async create(createProjectDto: CreateProjectDto): Promise<ProjectResponseDto> {
        const project = this.projectsRepository.create({ name: createProjectDto.name, ticketUrlTemplate: null });
    ...
    async findAllRequirements(projectId: string): Promise<RequirementResponseDto[]> {
        await this.getProjectOrThrow(projectId);
    ...
    async updateRequirement(
        ...
        const revisionMetadata = this.createRequirementRevisionMetadata(requirement, updateRequirementDto, actor);
    ...
    async listImplementationTickets(
        ...
        return tickets.map((ticket) =>
            this.requirementMapper.fromImplementationTicket(ticket, project.ticketUrlTemplate),
        );
    }
    ...
    async updateImplementationTicket(
        ...
        const savedTicket = await this.implementationTicketsRepository.save(ticket);
        requirement.implementationTickets = requirement.implementationTickets.map((item) =>
    ...
    async deleteImplementationTicket(
        ...
        requirement.implementationTickets = requirement.implementationTickets.filter(
    ...
    async findRequirementRevisions(projectId: string, requirementId: string): Promise<RequirementResponseDto[]> {
        await this.getProjectOrThrow(projectId);
        const requirement = await this.getRequirementOrThrow(projectId, requirementId);
    ...
    async compareRequirementRevisions(
        ...
        if (!Number.isInteger(fromRevision) || !Number.isInteger(toRevision) || fromRevision < 1 || toRevision < 1) {
            throw new BadRequestException('Revision comparison requires positive integer revision numbers.');
        }

        const snapshots = await this.findRequirementRevisions(projectId, requirementId);
        const from = snapshots.find((snapshot) => snapshot.revisionNumber === fromRevision);
    ...
    private async ensureTicketIdAvailable(requirementId: string, ticketId: string, excludedId?: string): Promise<void> {
        ...
        if (duplicate !== null)
            throw new BadRequestException(`Implementation ticket "${ticketId}" already exists for this requirement.`);
    }
    ...
    private createRequirementRevisionMetadata(
        ...
        if (update.status !== undefined) {
            if (update.changeReason !== undefined) {
                throw new BadRequestException('Change reason is derived by the server for lifecycle transitions.');
            }

            return createRevisionMetadata(statusChangeType(update), statusChangeReason(update), actor);
        }

        if (update.changeReason === undefined || update.changeReason.trim().length === 0) {
            throw new BadRequestException('Change reason is required when changing requirement content or metadata.');
        }
    ...
    private async prepareRequirementContentChange(
        ...
        if (requirement.status !== RequirementStatus.Draft && requirement.status !== RequirementStatus.Approved) {
    ...
    private ensureGenericStatusChangeAllowed(updateRequirementDto: UpdateRequirementDto): void {
        if (
            updateRequirementDto.status === RequirementStatus.Approved
            || updateRequirementDto.status === RequirementStatus.Rejected
        ) {
            throw new BadRequestException('Requirement approval and rejection must use the review decision endpoints.');
        }
    }
