...
export type ProjectsApiFixture = Readonly<{
    approveRequirement(projectId: string, requirementId: string, reviewer?: string): Promise<RequirementResponseBody>;
...
export type CategorySetup = Readonly<{ project: ProjectResponseBody; category: CategoryResponseBody }>;
...
export const test = base.extend<ProjectsApiFixtures>({
    resetDatabase: [
        ...
        async ({}, use, testInfo) => {
            ...
            const isAuthenticationSpec = normalizedTestFile.endsWith('/auth/auth.e2e-spec.ts');
    ...
    api: async ({ request }, use) => {
        await use({
            approveRequirement: (projectId: string, requirementId: string, reviewer?: string) =>
            ...
            createImplementationTicket: (
                ...
                ticketId: string,
                completedBy?: string,
            ) => createImplementationTicket(request, projectId, requirementId, ticketId, completedBy),
            listCategories: (projectId: string) => listCategories(request, projectId),
        });
    },
    project: async ({ api }, use) => {
        await use(await api.createProject(`Playwright API project fixture ${randomUUID()}`));
    },
