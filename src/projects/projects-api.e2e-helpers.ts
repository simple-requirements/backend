...
export async function createProject(request: APIRequestContext, name: string): Promise<ProjectResponseBody> {
    ...
    const memberships = await Promise.all([
        ...
        request.put(`/admin/projects/${project.id}/memberships/${E2E_DEVELOPER_USER_ID}`, {
            headers: E2E_ADMIN_HEADERS,
        }),
        request.put(`/admin/projects/${project.id}/memberships/${E2E_VIEWER_USER_ID}`, { headers: E2E_ADMIN_HEADERS }),
    ]);
...
export async function approveRequirement(
    ...
    const response = await request.post(`/projects/${projectId}/requirements/${requirementId}/review/approve`, {
        data: { reviewer },
        headers: E2E_REQUIREMENTS_ENGINEER_HEADERS,
    });
...
export async function createImplementationTicket(
    ...
    const response = await request.post(`/projects/${projectId}/requirements/${requirementId}/implementation-tickets`, {
        data: { ticketId, completedBy, completedAt: '2026-08-25' },
        headers: E2E_REQUIREMENTS_ENGINEER_HEADERS,
    });
