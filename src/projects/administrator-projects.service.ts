...
export class AdministratorProjectsService {
    ...
    async find(projectId: string): Promise<AdministratorProjectSummaryResponseDto> {
        return this.toSummary(await this.projects.findOne(projectId));
    }

    async create(createProjectDto: CreateProjectDto): Promise<AdministratorProjectSummaryResponseDto> {
    ...
    async update(
        ...
        return this.toSummary(await this.projects.update(projectId, updateProjectDto));
    }
    ...
    private async toSummary(project: ProjectResponseDto): Promise<AdministratorProjectSummaryResponseDto> {
        ...
        const categorySummaries = await Promise.all(
            categories.map(async ({ id, name }) => ({
                name,
                requirementCount: await this.requirements.countBy({ projectId: project.id, categoryId: id }),
            })),
        );
        const requirementCount = categorySummaries.reduce((total, category) => total + category.requirementCount, 0);
