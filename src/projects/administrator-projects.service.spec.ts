...
function projectResponse(overrides: Partial<ProjectResponseDto> = {}): ProjectResponseDto {
    return {
        id: PROJECT_ID,
        name: 'Admin project',
...
describe('AdministratorProjectsService', () => {
    const projects = { findAll: vi.fn(), findOne: vi.fn(), create: vi.fn(), update: vi.fn(), delete: vi.fn() };
    ...
    beforeEach(() => {
        ...
        requirements.countBy.mockImplementation(({ categoryId }: { categoryId: string }) =>
            Promise.resolve(categoryId === 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa' ? 4 : 3),
        );
    ...
    it('returns an empty summary for a newly created project.', async () => {
        projects.create.mockResolvedValue(projectResponse({ ticketUrlTemplate: null }));
        categories.find.mockResolvedValue([]);
    ...
    it('delegates administrative updates and deletion to the project domain service.', async () => {
        ...
        expect(projects.update).toHaveBeenCalledWith(PROJECT_ID, { name: 'Renamed' });
