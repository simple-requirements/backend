...
test.describe('Categories API - GET /projects/{projectId}/categories', () => {
    ...
    test('returns 403 when the user has no project-content access.', async ({ request }) => {
        ...
        const response = await request.get(`/projects/${unknownProjectId}/categories`, { headers: E2E_ADMIN_HEADERS });

        expect(response.status()).toBe(403);

        const body = (await response.json()) as ErrorResponseBody;

        expectErrorResponseBody(body, 403, 'Project access is not permitted.', 'Forbidden');
    });
});

test.describe('Categories API - POST /projects/{projectId}/categories', () => {
    test('creates a category.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API create category project ${randomUUID()}`);
    ...
    test('trims the category name.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API trim category project ${randomUUID()}`);
    ...
    test('trims and uppercases the category key.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API normalize key project ${randomUUID()}`);
    ...
    test('returns 403 when the user has no access to the project.', async ({ request }) => {
        const unknownProjectId = randomUUID();
    ...
    test('rejects an empty category name.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API empty category name project ${randomUUID()}`);
    ...
    for (const { key, testName } of INVALID_CATEGORY_KEYS) {
        test(`rejects an invalid category key because it is ${testName}.`, async ({ request, api }) => {
            ...
            expectErrorResponseBody(body, 400, 'Category key must contain 2 to 4 uppercase letters.', 'Bad Request');
        });
    }

    test('rejects an invalid category type.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API invalid category type project ${randomUUID()}`);
    ...
    test('rejects a duplicate category name within the same project.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API duplicate category name project ${randomUUID()}`);
    ...
    test('rejects a duplicate category key within the same project.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API duplicate category key project ${randomUUID()}`);

        await api.createCategory(project.id, `Playwright API first key category ${randomUUID()}`, 'DUP');

        const response = await request.post(`/projects/${project.id}/categories`, {
            data: { name: `Playwright API second key category ${randomUUID()}`, key: 'DUP', type: CategoryType.NFR },
        });
    ...
    test('allows the same category name in a different project.', async ({ api }) => {
        ...
        const firstCategory = await api.createCategory(firstProject.id, categoryName, 'ONE');
        const secondCategory = await api.createCategory(secondProject.id, categoryName, 'TWO');
...
test.describe('Categories API - PATCH /projects/{projectId}/categories/{id}', () => {
    ...
    test('updates name, key, and type together.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API update full category project ${randomUUID()}`);
        const category = await api.createCategory(project.id, 'Old category', 'OLD', CategoryType.FR);
    ...
    test('trims the updated category name.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API trim updated name project ${randomUUID()}`);
        const category = await api.createCategory(project.id, 'Old category', 'OLD');
    ...
    test('returns 404 when the category does not exist in the project.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API missing category project ${randomUUID()}`);
        const unknownCategoryId = randomUUID();
    ...
    for (const { key, testName } of INVALID_CATEGORY_KEYS) {
        test(`rejects an invalid updated key because it is ${testName}.`, async ({ request, api }) => {
            const project = await api.createProject(
    ...
    test('rejects an invalid updated type.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API invalid updated type project ${randomUUID()}`);
        const category = await api.createCategory(project.id, 'Authentication', 'AUTH');
    ...
    test('allows keeping the same name and key on the same category.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API keep same category project ${randomUUID()}`);
        const category = await api.createCategory(project.id, 'Authentication', 'AUTH');
...
test.describe('Categories API - DELETE /projects/{projectId}/categories/{id}', () => {
    test('deletes a category.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API delete category project ${randomUUID()}`);
        const category = await api.createCategory(project.id, 'Authentication', 'AUTH');
    ...
    test('does not list a deleted category.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API list after delete project ${randomUUID()}`);
        const category = await api.createCategory(project.id, 'Authentication', 'AUTH');
    ...
    test('returns 404 when the category does not exist in the project.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API delete unknown category project ${randomUUID()}`);
        const unknownCategoryId = randomUUID();
