...
test.describe("Projects API - GET /projects", () => {
  test("lists seeded demo projects for the Requirements Engineer.", async ({
    request,
  }) => {
  ...
  test("does not expose the project workspace list to an Administrator.", async ({
    request,
  }) => {
    const response = await request.get("/projects", {
...
test.describe("Projects API - GET /projects/{id}", () => {
  ...
  test("denies Administrator access to project details.", async ({
    request,
    api,
  }) => {
    const createdProject = await api.createProject(
      `Administrator-hidden project ${randomUUID()}`,
    );
...
test.describe("Project administration API - GET /admin/projects", () => {
  test("returns summary-only metadata to an Administrator.", async ({
    request,
    api,
  }) => {
    const project = await api.createProject(
      `Administrator summary project ${randomUUID()}`,
    );
    ...
    const listResponse = await request.get("/admin/projects", {
      headers: E2E_ADMIN_HEADERS,
    });
    ...
    const detailResponse = await request.get(`/admin/projects/${project.id}`, {
      headers: E2E_ADMIN_HEADERS,
    });
  ...
  test("denies project-scoped accounts access to Administrator summaries.", async ({
    request,
    project,
  }) => {
...
test.describe("Projects API - administration boundary", () => {
  test("does not expose project creation through the project workspace API.", async ({
    request,
  }) => {
...
test.describe("Project administration API - PATCH /admin/projects/{id}", () => {
  test("updates a project and trims the name.", async ({ request, api }) => {
    ...
    const ticketUrlTemplate =
      "https://tracker.example/projects/requirements/{ticket-id}";
  ...
  test("returns 404 when updating an unknown project.", async ({ request }) => {
    ...
    const response = await request.patch(
      `/admin/projects/${unknownProjectId}`,
      {
...
test.describe("Project administration API - DELETE /admin/projects/{id}", () => {
  test("deletes a project.", async ({ request, api }) => {
    ...
    const projects =
      (await listResponse.json()) as readonly ProjectResponseBody[];
  ...
  test("rejects deletion of a project that contains requirements.", async ({
    request,
    api,
  }) => {
    const project = await api.createProject(
      `Playwright API retained project ${randomUUID()}`,
    );
    ...
    const requirementResponse = await request.get(
