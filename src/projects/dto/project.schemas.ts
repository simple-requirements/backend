...
const ticketUrlTemplateSchema = z
    ...
    .refine((value) => value === null || value.split('{ticket-id}').length === 2, {
...
export const updateProjectSchema: ZodValidationSchema<UpdateProjectDto> = projectRequestBodySchema.pipe(
    z
        .object({ name: projectNameSchema.optional(), ticketUrlTemplate: ticketUrlTemplateSchema.optional() })
        .refine((value) => value.name !== undefined || value.ticketUrlTemplate !== undefined, {
...
export const implementationTicketSchema: ZodValidationSchema<UpsertImplementationTicketDto> =
    requirementRequestBodySchema.pipe(
        z.object({
            ticketId: trimmedStringSchema('Ticket ID must be a string.', 'Ticket ID must not be empty.').pipe(
                z.string().max(120),
            ),
            completedBy: trimmedStringSchema('Completed by must be a string.', 'Completed by must not be empty.').pipe(
                z.string().max(120),
            ),
...
export const updateRequirementSchema: ZodValidationSchema<UpdateRequirementDto> = requirementRequestBodySchema.pipe(
    z
        ...
        .refine(
            ...
            { message: 'Requirement approval and rejection must use the review decision endpoints.', path: ['status'] },
        )
        .refine(
            (requirementPatch) =>
        ...
        .refine(
            (requirementPatch) =>
                requirementPatch.status !== undefined || requirementPatch[CHANGE_REASON_FIELD] !== undefined,
            {
