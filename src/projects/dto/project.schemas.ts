...
const projectNameSchema = trimmedStringSchema(
...
const ticketUrlTemplateSchema = z
  .custom<string | null>(
    (value): value is string | null =>
      value === null || typeof value === "string",
    { message: "Ticket URL template must be a string or null." },
  )
  .transform((value) =>
    value === null || value.trim() === "" ? null : value.trim(),
  )
  .refine(
    (value) => value === null || value.split("{ticket-id}").length === 2,
    { message: "Ticket URL template must contain {ticket-id} exactly once." },
  )
  .refine(
    (value) => {
    ...
    {
      message:
        "Ticket URL template must produce an absolute HTTP or HTTPS URL.",
    },
  );
const categoryNameSchema = trimmedStringSchema(
...
const categoryTypeSchema = z.custom<CategoryType>(
  (value): value is CategoryType =>
    value === CategoryType.FR || value === CategoryType.NFR,
  {
...
const requirementTextSchema = nullableTrimmedStringSchema(
  "Requirement text fields must be strings or null.",
);
const reviewerSchema = nullableTrimmedStringSchema(
  "Reviewer must be a string or null.",
);
const obsoletedBySchema = nullableTrimmedStringSchema(
  "Obsoleted by must be a string or null.",
);
const rejectionReasonSchema = nullableTrimmedStringSchema(
  "Rejection reason must be a string or null.",
);
const obsolescenceReasonSchema = nullableTrimmedStringSchema(
  "Obsolescence reason must be a string or null.",
);
const changeReasonSchema = trimmedStringSchema(
  "Change reason must be a string.",
...
const projectRequestBodySchema = requestBodySchema(
  "Project request body must be an object.",
);
const categoryRequestBodySchema = requestBodySchema(
  "Category request body must be an object.",
);
const requirementRequestBodySchema = requestBodySchema(
  "Requirement request body must be an object.",
);

export const createProjectSchema: ZodValidationSchema<CreateProjectDto> =
  projectRequestBodySchema.pipe(z.object({ name: projectNameSchema }));

export const updateProjectSchema: ZodValidationSchema<UpdateProjectDto> =
  projectRequestBodySchema.pipe(
    z
      .object({
        name: projectNameSchema.optional(),
        ticketUrlTemplate: ticketUrlTemplateSchema.optional(),
      })
      .refine(
        (value) =>
          value.name !== undefined || value.ticketUrlTemplate !== undefined,
        { message: "At least one project field must be provided." },
      ),
  );

export const implementationTicketSchema: ZodValidationSchema<UpsertImplementationTicketDto> =
  requirementRequestBodySchema.pipe(
    z.object({
      ticketId: trimmedStringSchema(
        "Ticket ID must be a string.",
        "Ticket ID must not be empty.",
      ).pipe(z.string().max(120)),
      completedBy: trimmedStringSchema(
        "Completed by must be a string.",
        "Completed by must not be empty.",
      ).pipe(z.string().max(120)),
      completedAt: z.iso.date("Completion date must use YYYY-MM-DD."),
    }),
  );

export const createCategorySchema: ZodValidationSchema<CreateCategoryDto> =
  categoryRequestBodySchema.pipe(
    z.object({
...
export const updateCategorySchema: ZodValidationSchema<UpdateCategoryDto> =
  categoryRequestBodySchema.pipe(
    z
      .object({
      ...
      .refine(
        (categoryPatch) =>
          categoryPatch.name !== undefined ||
          categoryPatch.key !== undefined ||
          categoryPatch.type !== undefined,
        {
...
export const createRequirementSchema: ZodValidationSchema<CreateRequirementDto> =
  requirementRequestBodySchema.pipe(
    z.object({
...
export const updateRequirementSchema: ZodValidationSchema<UpdateRequirementDto> =
  requirementRequestBodySchema.pipe(
    z
      .object({
      ...
      .refine(
        (requirementPatch) =>
          requirementPatch.status !== undefined ||
          !hasAnyField(requirementPatch, STATUS_UPDATE_FIELDS),
        {
          message:
            "Requirement status metadata can only be changed together with a status change.",
        },
      )
      .refine(
        (requirementPatch) =>
          requirementPatch.status !== RequirementStatus.Approved &&
          requirementPatch.status !== RequirementStatus.Rejected,
        {
          message:
      ...
      .refine(
        (requirementPatch) =>
          requirementPatch.status !== undefined ||
          requirementPatch[CHANGE_REASON_FIELD] !== undefined,
        {
          message:
