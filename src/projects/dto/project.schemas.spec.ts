...
describe('project request schemas', () => {
    ...
    it('accepts an absolute ticket URL template with exactly one placeholder.', () => {
        expect(
            updateProjectSchema.parse({ ticketUrlTemplate: ' https://github.com/acme/issues/{ticket-id} ' }),
    ...
    it('rejects a ticket URL template without the ticket placeholder.', () => {
        expect(updateProjectSchema.safeParse({ ticketUrlTemplate: 'https://github.com/acme/issues' }).success).toBe(
            false,
        );
    });
    ...
    it('normalizes all user-entered implementation-ticket fields.', () => {
        ...
        expect(
            implementationTicketSchema.safeParse({
                ticketId: ' SOLAR-4712 ',
                completedBy: '   ',
    ...
    it('requires and normalizes a change reason for requirement content updates.', () => {
        ...
        expect(
            missingReason.error.issues.some(
                (issue) => issue.message === 'Change reason is required when changing requirement content or metadata.',
            ),
        ).toBe(true);
    });
    ...
    it('rejects client-supplied change reasons for lifecycle transitions.', () => {
        ...
        expect(
            result.error.issues.some(
                (issue) => issue.message === 'Change reason is derived by the server for lifecycle transitions.',
            ),
        ).toBe(true);
    });

    it('allows implementation status patches through the generic update endpoint.', () => {
        const result = updateRequirementSchema.parse({ status: RequirementStatus.Implemented });

        expect(result).toEqual({ status: RequirementStatus.Implemented });
    });

    it.each([RequirementStatus.Approved, RequirementStatus.Rejected])(
        'requires the dedicated review decision endpoint for %s status patches.',
        (status) => {
            const result = updateRequirementSchema.safeParse({
                status,
                reviewer: 'Jane Reviewer',
            ...
            expect(
                result.error.issues.some(
                    (issue) =>
                        issue.message === 'Requirement approval and rejection must use the review decision endpoints.',
                ),
            ).toBe(true);
        },
    );
    ...
    it('rejects a requirement patch that mixes content and status changes.', () => {
        ...
        expect(
            result.error.issues.some(
                (issue) => issue.message === 'Requirement content changes and status changes must be sent separately.',
            ),
        ).toBe(true);
    });
});
