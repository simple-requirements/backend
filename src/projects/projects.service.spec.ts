...
function createRequirementRevisionEntity(overrides: Partial<RequirementRevision> = {}): RequirementRevision {
...
describe('ProjectsService', () => {
    ...
    describe('creates', () => {
        it('a project.', async () => {
            ...
            expect(projectsRepository.create).toHaveBeenCalledWith({ name: 'Test project', ticketUrlTemplate: null });
    ...
    describe('deletes', () => {
        it('an empty project.', async () => {
            ...
            expect(requirementsRepository.existsBy).toHaveBeenCalledWith({ projectId });
    ...
    describe('requirements', () => {
        ...
        describe('findAllRequirements', () => {
            it('lists visible requirements sorted by visible key.', async () => {
                ...
                projectsRepository.findOne.mockResolvedValue(createProjectEntity());
                requirementsRepository.find.mockResolvedValue([firstRequirement, secondRequirement]);
                ...
                expect(result.map((requirement) => requirement.visibleKey)).toEqual(['FR-AUTH-0001', 'FR-AUTH-0002']);
            });
        });
        ...
        describe('findRequirement', () => {
            it('returns the current requirement.', async () => {
                ...
                const result = await service.findRequirement(PROJECT_ID, REQUIREMENT_ID);
        ...
        describe('findRequirementRevisions', () => {
            it('returns archived revisions and the current requirement.', async () => {
                ...
                requirementRevisionsRepository.find.mockResolvedValue([firstRevision, secondRevision]);

                const result = await service.findRequirementRevisions(PROJECT_ID, REQUIREMENT_ID);
                ...
                expect(result).toMatchObject([
                    ...
                    { id: REQUIREMENT_ID, revisionNumber: 3, status: RequirementStatus.Draft },
                ]);
            });
        });
        ...
        describe('updateRequirement', () => {
            it('creates a content revision while preserving an approved requirement lifecycle state.', async () => {
                ...
                expect(requirementRevisionsRepository.save).toHaveBeenCalledWith(revision);
                expect(existingRequirement.revisionNumber).toBe(3);
                expect(existingRequirement.status).toBe(RequirementStatus.Approved);
                expect(existingRequirement.description).toBe('Users must sign in with MFA.');
                expect(existingRequirement.changeReason).toBe('Clarified authentication behavior.');
                ...
                async (status) => {
                    ...
                    await expect(
                        service.updateRequirement(PROJECT_ID, REQUIREMENT_ID, {
                            status,
                            reviewer: 'Jane Reviewer',
                            ...(status === RequirementStatus.Rejected ? { rejectionReason: 'Not suitable.' } : {}),
                        }),
                    ).rejects.toThrow('Requirement approval and rejection must use the review decision endpoints.');
            ...
            it('requires an implementation ticket before an approved requirement can be implemented.', async () => {
                ...
                await expect(
                    service.updateRequirement(PROJECT_ID, REQUIREMENT_ID, { status: RequirementStatus.Implemented }),
