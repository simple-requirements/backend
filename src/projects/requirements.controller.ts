...
import { createRequirementSchema, updateRequirementSchema } from '@/projects/dto/project.schemas';
...
export class RequirementsController {
    ...
    async findAllRequirements(@Param('projectId') projectId: string): Promise<RequirementResponseDto[]> {
    ...
    async findRequirementRevisions(
        ...
        return this.projectsService.findRequirementRevisions(projectId, requirementId);
    }

    @Get(':requirementId/revisions/compare')
    @RequireProjectPermission(ProjectPermission.Read)
    @ApiOperation({ operationId: 'compareRequirementRevisions', summary: 'Compare two requirement revisions.' })
    @ApiOkResponse({
        description: 'Differences between the requested requirement revisions.',
        type: RequirementRevisionComparisonDto,
    })
    @ApiQuery({ name: 'from', type: Number, required: true, description: 'Source revision number.' })
    @ApiQuery({ name: 'to', type: Number, required: true, description: 'Target revision number.' })
    @ApiBadRequestResponse({ description: 'Revision numbers must be positive integers.' })
    @ApiNotFoundResponse({ description: 'The requirement or a requested revision was not found.' })
    ...
    @Get(':requirementId')
    @RequireProjectPermission(ProjectPermission.Read)
    @ApiOperation({ operationId: 'getRequirement', summary: 'Get one requirement.' })
    @ApiParam({ name: 'projectId', description: 'Project identifier.' })
    @ApiParam({ name: 'requirementId', description: 'Requirement identifier.' })
    @ApiOkResponse({ description: 'The current requirement.', type: RequirementResponseDto })
