...
export class RequirementLifecycleService {
    validateStatusChange(requirement: Requirement, update: UpdateRequirementDto): void {
        switch (update.status) {
            ...
            case RequirementStatus.Rejected:
                ...
                if (update.rejectionReason === undefined || update.rejectionReason === null)
    ...
    applyStatusChange(requirement: Requirement, update: UpdateRequirementDto): void {
        switch (update.status) {
            case RequirementStatus.Approved:
            ...
            case RequirementStatus.Implemented:
                ...
                return;
            case RequirementStatus.Obsolete: {
                const [obsoletedBy, reason] = this.getObsolescenceMetadata(requirement, update);
    ...
    private requireDraft(requirement: Requirement, action: string): void {
        if (requirement.status !== RequirementStatus.Draft)
            throw new BadRequestException(`Requirement in status "${requirement.status}" cannot be ${action}.`);
    }

    private requireReviewer(reviewer: string | null | undefined, action: string): void {
        if (reviewer === undefined || reviewer === null)
            throw new BadRequestException(`Reviewer must be provided when ${action} a requirement.`);
    }

    private getObsolescenceMetadata(requirement: Requirement, update: UpdateRequirementDto): readonly [string, string] {
        ...
        if (update.obsolescenceReason === undefined || update.obsolescenceReason === null)
