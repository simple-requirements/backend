...
export class AdministratorProjectsController {
    ...
    @Get()
    @ApiOperation({ operationId: 'adminListProjects', summary: 'List projects as administrative summaries.' })
    @ApiOkResponse({ type: AdministratorProjectSummaryResponseDto, isArray: true })
    ...
    @Get(':projectId')
    @ApiOperation({ operationId: 'adminGetProject', summary: 'Get one administrative project summary.' })
    ...
    find(@Param('projectId', ParseUUIDPipe) projectId: string): Promise<AdministratorProjectSummaryResponseDto> {
        return this.projects.find(projectId);
    }

    @Post()
    @ApiOperation({ operationId: 'adminCreateProject', summary: 'Create a project.' })
    ...
    @Patch(':projectId')
    @ApiOperation({ operationId: 'adminUpdateProject', summary: 'Update project administration settings.' })
    @ApiParam({ name: 'projectId', description: 'Project identifier.' })
    @ApiOkResponse({ type: AdministratorProjectSummaryResponseDto })
    @ApiBadRequestResponse({ description: 'The request body is invalid.' })
    @ApiUnauthorizedResponse({ description: 'Authentication is required.' })
    ...
    @ApiOperation({ operationId: 'adminDeleteProject', summary: 'Delete a project.' })
    @ApiParam({ name: 'projectId', description: 'Project identifier.' })
    @ApiNoContentResponse({ description: 'The empty project was deleted.' })
    @ApiBadRequestResponse({ description: 'The project contains requirements and must be retained.' })
    @ApiUnauthorizedResponse({ description: 'Authentication is required.' })
    @ApiForbiddenResponse({ description: 'Administrator access is required.' })
    @ApiNotFoundResponse({ description: 'The project was not found.' })
