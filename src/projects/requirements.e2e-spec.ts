...
test.describe('Requirements API - DELETE /projects/{projectId}/requirements/{requirementId}', () => {
    test('moves draft requirements into and out of the recycle bin.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API recycle bin project ${randomUUID()}`);
        const category = await api.createCategory(project.id, 'Authentication', 'AUTH', CategoryType.FR);
        const firstRequirement = await api.createRequirement(project.id, category.id, 'First draft.');
        const secondRequirement = await api.createRequirement(project.id, category.id, 'Second draft.');

        const softDeleteResponse = await request.delete(`/projects/${project.id}/requirements/${firstRequirement.id}`);

        expect(softDeleteResponse.status()).toBe(204);
        expect(await softDeleteResponse.text()).toBe('');

        const visibleListResponse = await request.get(`/projects/${project.id}/requirements`);
        const visibleListBody = (await visibleListResponse.json()) as readonly RequirementResponseBody[];

        expect(visibleListBody.map((listedRequirement) => listedRequirement.id)).not.toContain(firstRequirement.id);
        expect(visibleListBody.map((listedRequirement) => listedRequirement.id)).toContain(secondRequirement.id);

        const deletedListResponse = await request.get(`/projects/${project.id}/requirements?deleted`);
        const deletedListBody = (await deletedListResponse.json()) as readonly RequirementResponseBody[];
    ...
    test('does not soft delete approved requirements.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API approved delete project ${randomUUID()}`);
        const category = await api.createCategory(project.id, 'Authentication', 'AUTH', CategoryType.FR);
        const requirement = await api.createRequirement(project.id, category.id);
        ...
        expectErrorResponseBody(body, 400, 'Only draft requirements can be deleted.', 'Bad Request');
    });
});

test.describe('Requirements API - DELETE /projects/{projectId}/requirements', () => {
    test('clears the complete requirement recycle bin.', async ({ request, api }) => {
        const project = await api.createProject(`Playwright API clear recycle bin project ${randomUUID()}`);
        const category = await api.createCategory(project.id, 'Authentication', 'AUTH', CategoryType.FR);
        const firstRequirement = await api.createRequirement(project.id, category.id, 'First draft.');
        const secondRequirement = await api.createRequirement(project.id, category.id, 'Second draft.');
        ...
        const clearResponse = await request.delete(`/projects/${project.id}/requirements?deleted`);
